...
const [activeTab, setActiveTab] = useState('credit');
  ...
  const { 
    ...
    queryFn: async () => {
      ...
      // Calculate totals
      const receivedTotal = response.data.received?.reduce(
        (sum, item) => sum + parseInt(item.received_amount || 0), 0
      ) || 0;
      
      const paymentTotal = response.data.payment?.reduce(
        (sum, item) => sum + parseInt(item.payment_amount || 0), 0
      ) || 0;
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: error.response?.data?.message || "Failed to update day book",
        variant: "destructive",
      });
    } finally {
