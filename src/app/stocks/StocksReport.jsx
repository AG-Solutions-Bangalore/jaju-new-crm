...
import { Printer, FileText, FileDown, Search, ChevronLeft, Plus, Pencil } from "lucide-react";
...
const StocksReport = () => {
  ...
  const updateProductMutation = useMutation({
    ...
    onError: (error) => {
      toast({ title: "Error", description: error.response?.data?.message || "Failed to rename item", variant: "destructive" });
    },
  });
  ...
  const handleEditItem = (itemName) => {
    const product = productTypes?.find(
      (p) => (p.product_type || p.item_name || "").toLowerCase() === itemName.toLowerCase()
    );
    if (!product) {
      toast({ title: "Not Found", description: "Item not found in product master. Create it first.", variant: "destructive" });
      return;
    }
                          ...
                          >
                            <td className="border p-1 text-left">
                              <span className="flex items-center gap-1">
                                {item.item_name}
                                <button onClick={() => handleEditItem(item.item_name)} className="text-gray-400 hover:text-blue-600">
                          ...
                          stocksData.stocks.map((item, index) => (
                            ...
                            >
                              <TableCell className="text-left border-r">
                                <span className="flex items-center gap-1">
                                  {item.item_name}
                                  <button onClick={() => handleEditItem(item.item_name)} className="text-gray-400 hover:text-blue-600 print:hidden">
                                    <Pencil className="h-3 w-3" />
                                  </button>
                                </span>
                              </TableCell>
      ...
      <Dialog open={showNewItemDialog} onOpenChange={setShowNewItemDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Add a new product type to the inventory.</DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreateItem} disabled={createProductMutation.isPending}>
      ...
      <Dialog open={showEditItemDialog} onOpenChange={setShowEditItemDialog}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleUpdateItem} disabled={updateProductMutation.isPending}>
